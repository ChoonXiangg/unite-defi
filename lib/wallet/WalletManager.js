...
class WalletManager {
  ...
  async importWalletFromPrivateKey(privateKey, type = 'evm') {
    try {
      if (type === 'evm') {
        // Import EVM wallet from private key
        const evmWallet = new ethers.Wallet(privateKey);
        
        // For EVM-only import, we can't generate a corresponding Tezos wallet
        // So we'll create a placeholder Tezos wallet or generate a new one
        const tempMnemonic = generateMnemonic();
        const seed = mnemonicToSeedSync(tempMnemonic);
        const tezosWallet = await this.createTezosWallet(seed);
        
        return {
          importType: 'evm_private_key',
          evm: {
            address: evmWallet.address,
            privateKey: evmWallet.privateKey,
            publicKey: evmWallet.publicKey
          ...
          note: 'Imported from EVM private key. Tezos wallet was generated separately.'
        };
      } else if (type === 'tezos') {
        // Import Tezos wallet from private key
        const signer = new InMemorySigner(privateKey);
        
        // For Tezos-only import, generate a separate EVM wallet
        const tempMnemonic = generateMnemonic();
        const evmWallet = ethers.Wallet.fromPhrase(tempMnemonic);
        
        return {
          importType: 'tezos_private_key',
          evm: {
            address: evmWallet.address,
            privateKey: evmWallet.privateKey,
            publicKey: evmWallet.publicKey
          ...
          note: 'Imported from Tezos private key. EVM wallet was generated separately.'
        };
      } else {
        throw new Error('Unsupported private key type. Use "evm" or "tezos".');
      }
    } catch (error) {
